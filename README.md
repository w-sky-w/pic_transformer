...
- 自动识别 **animated WebP**（多帧 WebP），并直接输出为 **GIF**。
- 即使输出格式选择的是 PNG/JPG，animated WebP 仍会优先按 GIF 导出，避免丢失动画。
...
- 输出文件名默认与原文件同名，仅后缀变化（animated WebP 会变为 `.gif`）。
...
已将默认窗口尺寸与最小尺寸调大。如果仍有显示缩放问题，可在 Windows 显示设置中将缩放比例调低，或手动拉大窗口。
